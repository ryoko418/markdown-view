...
import (
	...
	handler "github.com/ryoko418/markdown-view/controlles/handler"
...
func (r *MarkdownViewReconciler) reconcileConfigMap(ctx context.Context, mdView viewv1.MarkdownView) error {
	...
	op, err := ctrl.CreateOrUpdate(ctx, r.Client, cm, func() error {
		podOwner.SetGroupVersionKind(schema.GroupVersionKind{Group: "", Kind: "MarkdownView"})
	        logger.Info("ConfigMap2222")

		return handler.SetOwnerAnnotations(podOwner, cm)
	})
	*/
	if err != nil {
...
func (r *MarkdownViewReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
	        		...
	        		&handler.EnqueueRequestForAnnotation{schema.GroupKind{Group: "", Kind: "ConfigMap"}},
		).
		Complete(r)
}
